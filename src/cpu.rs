    ...
    pub fn realize(&self) -> Tensor {
        match self {
            ...
            UnrealizedOp::Conv2D(t, kernel, strides, groups) => {
                ...
                let t = t.realize();
                let t_data = t.data.unwrap();
                let kernel = kernel.realize();
                let kernel_data = kernel.data.unwrap();
                ...
                Tensor::new(
                    self.clone(),
                    output_data,
                    vec![n, c_out, output_height, output_width],
                )
            }
            UnrealizedOp::Pad2D(t, value, padding) => {
                ...
                Tensor::new(self.clone(), new_data.clone(), new_shape)
            }
